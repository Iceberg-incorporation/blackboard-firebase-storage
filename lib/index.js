...
BlackboardFirebaseStorage.prototype.connect = function (config, callback) {
    ...
    this.firebase = firebase.initializeApp(this.firebaseConfig[0]);
    ...
    if (typeof callback !== 'function') {
        ...
        if (config) {
            callback(null, this.firebaseConfig[0])
        } else {
...
BlackboardFirebaseStorage.prototype.uploadFile = function (bucket, name, data, callback) {
    ...
    storageRef.child(`${bucket}/${name}`).put(data).then(function (snapshot) {
...
BlackboardFirebaseStorage.prototype.getUrl = function (bucket, name, callback) {
    const storageRef = this.storageRef
    storageRef.child(`${bucket}/${name}`).getDownloadURL().then(function (snapshot) {
        if (typeof callback !== 'function') {
...
BlackboardFirebaseStorage.prototype.getFile = function (bucket, name, callback) {
    const storageRef = this.storageRef
    storageRef.child(`${bucket}/${name}`).getMetadata().then(function (snapshot) {
        if (typeof callback !== 'function') {
...
BlackboardFirebaseStorage.prototype.remove = function (bucket, name, callback) {
    const storageRef = this.storageRef
    storageRef.child(`${bucket}/${name}`).delete().then(function (snapshot) {
        if (typeof callback !== 'function') {
